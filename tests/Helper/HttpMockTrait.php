...
trait HttpMockTrait
{
    ...
    {
        $this->mockHttpClient = new MockHttpClient($this->createMockResponseCallback());
        return $this->mockHttpClient;
    }

    protected function createMockResponseCallback(): callable
    {
    ...
    {
        $rates = \App\Tests\Helper\TestConstants::DEFAULT_RATES;
        return isset($rates[$symbol]) ? (float) $rates[$symbol] : 100000.0;
    }

    protected function mockBinanceApiResponse(array $klineData): void
    {
        $this->mockHttpClient = new MockHttpClient([
            new MockResponse(json_encode($klineData))
        ]);
    }

    protected function mockBinanceApiError(string $error = 'API Error'): void
    {
        $this->mockHttpClient = new MockHttpClient([
            new MockResponse('{"error": "' . $error . '"}', ['http_code' => 500])
        ]);
    }
}